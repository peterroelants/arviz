(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("8a07f756-54b1-45aa-a18c-de147ec62886");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '8a07f756-54b1-45aa-a18c-de147ec62886' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.0.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"9e7ca9ca-422f-4b2e-82ab-e58f400cd106":{"defs":[],"roots":{"references":[{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17912","type":"Line"},{"attributes":{},"id":"17930","type":"UnionRenderers"},{"attributes":{},"id":"17894","type":"WheelZoomTool"},{"attributes":{"below":[{"id":"17883"}],"center":[{"id":"17886"},{"id":"17890"},{"id":"17936"}],"height":500,"left":[{"id":"17887"}],"output_backend":"webgl","renderers":[{"id":"17914"},{"id":"17920"},{"id":"17942"}],"title":{"id":"17944"},"toolbar":{"id":"17901"},"toolbar_location":null,"width":500,"x_range":{"id":"17875"},"x_scale":{"id":"17879"},"y_range":{"id":"17877"},"y_scale":{"id":"17881"}},"id":"17874","subtype":"Figure","type":"Plot"},{"attributes":{"coordinates":null,"data_source":{"id":"17910"},"glyph":{"id":"17911"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17913"},"nonselection_glyph":{"id":"17912"},"view":{"id":"17915"}},"id":"17914","type":"GlyphRenderer"},{"attributes":{"overlay":{"id":"17899"}},"id":"17893","type":"BoxZoomTool"},{"attributes":{"coordinates":null,"group":null,"items":[{"id":"17937"}]},"id":"17936","type":"Legend"},{"attributes":{},"id":"17875","type":"DataRange1d"},{"attributes":{"toolbar":{"id":"17963"},"toolbar_location":"above"},"id":"17964","type":"ToolbarBox"},{"attributes":{},"id":"17892","type":"PanTool"},{"attributes":{"children":[[{"id":"17874"},0,0]]},"id":"17962","type":"GridBox"},{"attributes":{"data":{},"selected":{"id":"17958"},"selection_policy":{"id":"17957"}},"id":"17938","type":"ColumnDataSource"},{"attributes":{},"id":"17881","type":"LinearScale"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17918","type":"Line"},{"attributes":{"source":{"id":"17916"}},"id":"17921","type":"CDSView"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17911","type":"Line"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17919","type":"Line"},{"attributes":{},"id":"17926","type":"AllLabels"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"17920"}]},"id":"17937","type":"LegendItem"},{"attributes":{"toolbars":[{"id":"17901"}],"tools":[{"id":"17891"},{"id":"17892"},{"id":"17893"},{"id":"17894"},{"id":"17895"},{"id":"17896"},{"id":"17897"},{"id":"17898"}]},"id":"17963","type":"ProxyToolbar"},{"attributes":{"fill_alpha":{"value":0.2},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.2},"line_alpha":{"value":0.2},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17941","type":"Circle"},{"attributes":{},"id":"17925","type":"BasicTickFormatter"},{"attributes":{"children":[{"id":"17964"},{"id":"17962"}]},"id":"17965","type":"Column"},{"attributes":{},"id":"17896","type":"UndoTool"},{"attributes":{},"id":"17877","type":"DataRange1d"},{"attributes":{"callback":null},"id":"17898","type":"HoverTool"},{"attributes":{},"id":"17929","type":"AllLabels"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"17899","type":"BoxAnnotation"},{"attributes":{"source":{"id":"17938"}},"id":"17943","type":"CDSView"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17917","type":"Line"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17939","type":"Circle"},{"attributes":{"coordinates":null,"data_source":{"id":"17938"},"glyph":{"id":"17939"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17941"},"nonselection_glyph":{"id":"17940"},"view":{"id":"17943"}},"id":"17942","type":"GlyphRenderer"},{"attributes":{},"id":"17928","type":"BasicTickFormatter"},{"attributes":{"source":{"id":"17910"}},"id":"17915","type":"CDSView"},{"attributes":{},"id":"17957","type":"UnionRenderers"},{"attributes":{},"id":"17897","type":"SaveTool"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17913","type":"Line"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"17944","type":"Title"},{"attributes":{},"id":"17933","type":"Selection"},{"attributes":{},"id":"17958","type":"Selection"},{"attributes":{"coordinates":null,"formatter":{"id":"17928"},"group":null,"major_label_policy":{"id":"17929"},"ticker":{"id":"17884"}},"id":"17883","type":"LinearAxis"},{"attributes":{},"id":"17891","type":"ResetTool"},{"attributes":{},"id":"17932","type":"UnionRenderers"},{"attributes":{"tools":[{"id":"17891"},{"id":"17892"},{"id":"17893"},{"id":"17894"},{"id":"17895"},{"id":"17896"},{"id":"17897"},{"id":"17898"}]},"id":"17901","type":"Toolbar"},{"attributes":{"coordinates":null,"data_source":{"id":"17916"},"glyph":{"id":"17917"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17919"},"nonselection_glyph":{"id":"17918"},"view":{"id":"17921"}},"id":"17920","type":"GlyphRenderer"},{"attributes":{"axis":{"id":"17883"},"coordinates":null,"group":null,"ticker":null},"id":"17886","type":"Grid"},{"attributes":{"coordinates":null,"formatter":{"id":"17925"},"group":null,"major_label_policy":{"id":"17926"},"ticker":{"id":"17888"}},"id":"17887","type":"LinearAxis"},{"attributes":{},"id":"17888","type":"BasicTicker"},{"attributes":{"data":{},"selected":{"id":"17933"},"selection_policy":{"id":"17932"}},"id":"17916","type":"ColumnDataSource"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"17931"},"selection_policy":{"id":"17930"}},"id":"17910","type":"ColumnDataSource"},{"attributes":{},"id":"17931","type":"Selection"},{"attributes":{},"id":"17884","type":"BasicTicker"},{"attributes":{"overlay":{"id":"17900"}},"id":"17895","type":"LassoSelectTool"},{"attributes":{"axis":{"id":"17887"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"17890","type":"Grid"},{"attributes":{},"id":"17879","type":"LinearScale"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.1},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17940","type":"Circle"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"17900","type":"PolyAnnotation"}],"root_ids":["17965"]},"title":"Bokeh Application","version":"2.4.0"}}';
                  const render_items = [{"docid":"9e7ca9ca-422f-4b2e-82ab-e58f400cd106","root_ids":["17965"],"roots":{"17965":"8a07f756-54b1-45aa-a18c-de147ec62886"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();