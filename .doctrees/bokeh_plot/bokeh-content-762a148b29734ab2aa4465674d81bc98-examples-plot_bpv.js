(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("c9f7326b-1e89-48fe-9dd1-3bf233f42c86");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid 'c9f7326b-1e89-48fe-9dd1-3bf233f42c86' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.0.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"53ebf6ea-a0c8-4dff-9cd1-b5bd9eff8b36":{"defs":[],"roots":{"references":[{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"17802"},"selection_policy":{"id":"17801"}},"id":"17773","type":"ColumnDataSource"},{"attributes":{},"id":"17805","type":"UnionRenderers"},{"attributes":{"coordinates":null,"data_source":{"id":"17780"},"glyph":{"id":"17781"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17783"},"nonselection_glyph":{"id":"17782"},"view":{"id":"17785"}},"id":"17784","type":"GlyphRenderer"},{"attributes":{},"id":"17806","type":"Selection"},{"attributes":{"coordinates":null,"formatter":{"id":"17799"},"group":null,"major_label_policy":{"id":"17800"},"ticker":{"id":"17747"}},"id":"17746","type":"LinearAxis"},{"attributes":{"bottom":1.459677659268402,"coordinates":null,"fill_alpha":0.1,"fill_color":"#2a2eec","group":null,"top":0.6850827603275131},"id":"17779","type":"BoxAnnotation"},{"attributes":{"tools":[{"id":"17754"},{"id":"17755"},{"id":"17756"},{"id":"17757"},{"id":"17758"},{"id":"17759"},{"id":"17760"},{"id":"17761"}]},"id":"17764","type":"Toolbar"},{"attributes":{},"id":"17801","type":"UnionRenderers"},{"attributes":{"overlay":{"id":"17763"}},"id":"17758","type":"LassoSelectTool"},{"attributes":{},"id":"17738","type":"DataRange1d"},{"attributes":{},"id":"17802","type":"Selection"},{"attributes":{"callback":null},"id":"17761","type":"HoverTool"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17774","type":"Line"},{"attributes":{},"id":"17747","type":"BasicTicker"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"17763","type":"PolyAnnotation"},{"attributes":{},"id":"17759","type":"UndoTool"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17776","type":"Line"},{"attributes":{},"id":"17760","type":"SaveTool"},{"attributes":{"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17781","type":"Line"},{"attributes":{},"id":"17757","type":"WheelZoomTool"},{"attributes":{"line_alpha":0.2,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17783","type":"Line"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17775","type":"Line"},{"attributes":{"below":[{"id":"17746"}],"center":[{"id":"17749"},{"id":"17753"},{"id":"17779"}],"height":500,"left":[{"id":"17750"}],"output_backend":"webgl","renderers":[{"id":"17777"},{"id":"17784"},{"id":"17790"}],"title":{"id":"17792"},"toolbar":{"id":"17764"},"toolbar_location":null,"width":500,"x_range":{"id":"17738"},"x_scale":{"id":"17742"},"y_range":{"id":"17740"},"y_scale":{"id":"17744"}},"id":"17737","subtype":"Figure","type":"Plot"},{"attributes":{"children":[[{"id":"17737"},0,0]]},"id":"17810","type":"GridBox"},{"attributes":{"coordinates":null,"data_source":{"id":"17786"},"glyph":{"id":"17787"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17789"},"nonselection_glyph":{"id":"17788"},"view":{"id":"17791"}},"id":"17790","type":"GlyphRenderer"},{"attributes":{"coordinates":null,"data_source":{"id":"17773"},"glyph":{"id":"17774"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17776"},"nonselection_glyph":{"id":"17775"},"view":{"id":"17778"}},"id":"17777","type":"GlyphRenderer"},{"attributes":{"data":{"x":[0,1],"y":[1,1]},"selected":{"id":"17804"},"selection_policy":{"id":"17803"}},"id":"17780","type":"ColumnDataSource"},{"attributes":{},"id":"17751","type":"BasicTicker"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"17762","type":"BoxAnnotation"},{"attributes":{"children":[{"id":"17812"},{"id":"17810"}]},"id":"17813","type":"Column"},{"attributes":{"data":{},"selected":{"id":"17806"},"selection_policy":{"id":"17805"}},"id":"17786","type":"ColumnDataSource"},{"attributes":{"overlay":{"id":"17762"}},"id":"17756","type":"BoxZoomTool"},{"attributes":{"source":{"id":"17780"}},"id":"17785","type":"CDSView"},{"attributes":{},"id":"17796","type":"BasicTickFormatter"},{"attributes":{},"id":"17803","type":"UnionRenderers"},{"attributes":{"axis":{"id":"17750"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"17753","type":"Grid"},{"attributes":{"line_alpha":0.1,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17782","type":"Line"},{"attributes":{},"id":"17742","type":"LinearScale"},{"attributes":{},"id":"17804","type":"Selection"},{"attributes":{},"id":"17744","type":"LinearScale"},{"attributes":{"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17787","type":"Line"},{"attributes":{},"id":"17797","type":"AllLabels"},{"attributes":{},"id":"17754","type":"ResetTool"},{"attributes":{"toolbars":[{"id":"17764"}],"tools":[{"id":"17754"},{"id":"17755"},{"id":"17756"},{"id":"17757"},{"id":"17758"},{"id":"17759"},{"id":"17760"},{"id":"17761"}]},"id":"17811","type":"ProxyToolbar"},{"attributes":{},"id":"17755","type":"PanTool"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"17792","type":"Title"},{"attributes":{},"id":"17740","type":"DataRange1d"},{"attributes":{"axis":{"id":"17746"},"coordinates":null,"group":null,"ticker":null},"id":"17749","type":"Grid"},{"attributes":{"coordinates":null,"formatter":{"id":"17796"},"group":null,"major_label_policy":{"id":"17797"},"ticker":{"id":"17751"}},"id":"17750","type":"LinearAxis"},{"attributes":{},"id":"17799","type":"BasicTickFormatter"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17788","type":"Line"},{"attributes":{"source":{"id":"17773"}},"id":"17778","type":"CDSView"},{"attributes":{},"id":"17800","type":"AllLabels"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17789","type":"Line"},{"attributes":{"toolbar":{"id":"17811"},"toolbar_location":"above"},"id":"17812","type":"ToolbarBox"},{"attributes":{"source":{"id":"17786"}},"id":"17791","type":"CDSView"}],"root_ids":["17813"]},"title":"Bokeh Application","version":"2.4.0"}}';
                  const render_items = [{"docid":"53ebf6ea-a0c8-4dff-9cd1-b5bd9eff8b36","root_ids":["17813"],"roots":{"17813":"c9f7326b-1e89-48fe-9dd1-3bf233f42c86"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();